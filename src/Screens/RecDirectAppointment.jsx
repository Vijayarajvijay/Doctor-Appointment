...
const RecDirectAppointment = ({ navigation }) => {
  const appointments = [
    {
      id: '1',
      status: 'Completed',
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={[styles.statusContainer, getStatusBackgroundColor(item.status)]}>
        <Text style={styles.statusText}> Consulting {item.status}</Text>
    ...
    borderColor:Colors.secondary,backgroundColor:"white"}]} onPress={() => navigation.navigate('AppointmentDetails', { appointmentId: item.id })}>
      ...
      <TouchableOpacity style={styles.detailsButton} onPress={() => navigation.navigate('Record Details', { appointmentId: item.id })}>
    ...
    <View style={styles.container}>
      {/* <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Icon name="arrowleft" size={24} color={Colors.secondary} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>My Appointment</Text>
        <TouchableOpacity onPress={() => navigation.navigate('EditBookings')}>
        </TouchableOpacity>
      </View> */}
...
export default RecDirectAppointment;
